...
export type Sheet = {game_id:string;player_id:string;name:string;scores:Scores;bonus:number;revision:number;completed_at:number|null};
...
export type TablePlayer = {id:string;name:string;active:boolean};
export type State = {me:{id:string;name:string};room:Room;rooms:Room[];players:TablePlayer[];game:Game;history:Game[];mode?:string};
...
export function leaderboard(history:Game[]){const rows=new Map<string,{id:string;name:string;games:number;wins:number;best:number;sum:number}>();for(const g of history){if(!g.ended_at||!g.sheets.length||g.sheets.some(s=>!s.completed_at))continue;const max=Math.max(...g.sheets.map(s=>totals(s.scores,s.bonus).total));for(const s of g.sheets){const t=totals(s.scores,s.bonus).total;const r=rows.get(s.player_id)||{id:s.player_id,name:s.name,games:0,wins:0,best:0,sum:0};r.name=s.name;r.games++;r.sum+=t;r.best=Math.max(r.best,t);if(g.sheets.length>1&&t===max)r.wins++;rows.set(s.player_id,r);}}return[...rows.values()].sort((a,b)=>b.wins-a.wins||b.best-a.best);}
