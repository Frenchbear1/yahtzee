...
const sql=new DatabaseSync(':memory:');sql.exec(readFileSync('drizzle/0000_confused_nova.sql','utf8'));
...
assert.equal(alice.game.id,bob.game.id);assert.equal(alice.game.sheets.length,2);
...
await call(a,{action:'score',gameId:alice.game.id,revision:mine(alice).revision,category:'yahtzee',value:0},400);
...
const before=alice.game.id;alice=await call(a,{action:'new-game',gameId:before,confirm:true});assert.equal(alice.history.find(g=>g.id===before).ended_at,null);
...
console.log(`PASS: ${checks} API checks: shared rooms, identity, validation, undo/clear, stale edits, bonuses, completion, history, access control, and new games.`);
sql.close();
